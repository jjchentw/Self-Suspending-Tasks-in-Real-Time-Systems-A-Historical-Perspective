...
previously-proposed strategies for handling self-suspending tasks
 ...
 computation demand induced by self-suspension, leading to incorrect results.
...
% \cline{1-6}
% \multirow{5}{*}{\rotatebox[origin=c]{90}{ interfering tasks ($hp(k)$ under FP)}}& 
...
\begin{tabular}{||C{2.5cm}|C{1cm}|C{3cm}||C{4cm}|C{6cm}|}  
  \hline
papers/methods & year & suspension and scheduling model& interfered task ($\tau_k$)& interfering tasks ($hp(k)$ under FP) \\%& flawed? & fixed?\\
\hline
...
\cline{2-6}
\multirow{4}{*}{\rotatebox[origin=c]{90}{ interfering tasks}}& carry-in jobs (Sec.~\ref{sec:model-carry-in})& \cite{LiuChen:rtss2014}, \cite{huangpass:dac2015}& \multicolumn{2}{c|}{\cite{Huang:multiseg}} & -\\
\cline{2-6}
...
As a result, if the computation segments and suspension intervals of $\tau_k$ interleave such that $\tau_k$ self-suspends only between the arrival of higher-priority jobs
...
The idea of the above test is based on a safe but rather pessimistic approach where each computation segment of task $\tau_k$ always suffers from the worst-case interference.
However, it may not be possible to construct such worst-case interference
for every computation segment of a job of task $\tau_k$ since the release patterns of the
higher priority tasks are also constrained by their temporal properties, shown in the following example:
\begin{example}
...
The reason why considering $R_3^1 + R_3^2 + S_3$ is pessimistic is that
a job of task $\tau_2$,  under such an analysis, is considered to interfere with both the first and the second
computation segments of a job of task $\tau_3$. However,  a job of task $\tau_2$ can
only interfere with one of the two segments of a job of task $\tau_3$ in any possible release patterns.
\hfill\myendproof  
\end{example} 
...
task $\tau_k$ under analysis.\footnote{It is in fact not clear why suspension induces blocking.  The authors in \cite{ChenECRTS2016-suspension} noted that ``Even though the authors in this paper are able to provide a proof to support the correctness, the authors are not able to provide any rationale behind this method which treats suspension time as blocking time.'' Here, we still use the original wording introduced by Jane Liu for consistency with the existing literature.} This solution, limited to fixed-priority scheduling policies, considers that a job of 
...
%Therefore, this work is discussed later, in Section~\ref{sec:multiprocessor-HRT}.
...
The period enforcer has been revisited by Chen and Brandenburg in \cite{ChenBrandenburg-2016-LITES}, with the following three observations:
\begin{enumerate}
...
enforcement can also be relaxed. It has been already
shown in \cite{Bruggen16RTNS,RTSS-ChenL14} that releasing its
