...
analyzing their schedulability and building effective suspension-aware scheduling algorithms. In this section, we provide an overview of the different 
...
As discussed in details in Section~\ref{sec:hardness}, performing the timing analysis for a set of self-suspending tasks has been proven to be intractable in the general case. For those reasons, most works adopt some common strategies to simplify the worst-case response time analysis of self-suspending tasks. We present those strategies in Section~\ref{sec:model-interferred} and Section~\ref{sec:model-interfering} by decoupling the modeling of the task under analysis (\ie, $\tau_2$ in the above example) and the task interfering with the analyzed task, respectively. Section~\ref{sec:release-enforce} presents  release enforcement mechanisms to reduce the impact due to self-suspension. Section~\ref{sec:multiprocessor-HRT} will shortly discuss how to handle self-suspending tasks in multiprocessor systems.


Note that we implicitly assume uniprocessor systems in these sections. The multiprocessor case is covered in Section~\ref{sec:multiprocessor-HRT}. In Sections~\ref{sec:model-interferred},~\ref{sec:model-interfering},~and~\ref{sec:release-enforce}, in most cases, we will use fixed-priority scheduling to explain the strategies. Therefore, we implicitly consider the timing analysis for task $\tau_k$, in which $hp(k)$ is the set of higher-priority tasks, if fixed-priority scheduling is considered. Table~\ref{tab:summary-methods} provides a summary to explain how the methods explained in this section are linked to the existing results in the literature.
...
\begin{table}[t]
  \centering
  \scalebox{0.65}{
\begin{tabular}{||C{2.5cm}|C{1cm}|C{3cm}||C{5cm}|C{5cm}|c|}  
  \hline
papers/methods & year & suspension and scheduling model& interfered task & interferring tasks & flawed\\
\hline
Rajkumar \cite{Raj:suspension1991} & 1991 & segmented, FP & \multicolumn{2}{C{10cm}|}{A period enforcer algorithm was proposed to artificially 
...
\hline
 Ming \cite{MingLiRTCSA1994} & 1994 & dynamic, FP & suspension as computation, Sec.~\ref{sec:model-interferred-oblivious} & as release jitter, Sec.~\ref{sec:model-interfering-jitter} & \checkmark\\
\hline
Kim et al. \cite{RTCSA-KimCPKH95} & 1995 & dynamic, FP & suspension as computation, Sec.~\ref{sec:model-interferred-oblivious} & as release jitter, Sec.~\ref{sec:model-interfering-jitter} & \checkmark\\
\hline
Palencia and Harbour~\cite{PH:rtss98} & 1998 & segmented, FP & suspension as computation, Sec.~\ref{sec:model-interferred-oblivious}  & segmented structures with dynamic offsets, Sec.~\ref{sec:model-interfering-improving} &\\
\hline
Liu~\cite[Pages 164-165]{Liu:2000:RS:518501} & 2000 & dynamic, FP & suspension as computation, Sec.~\ref{sec:model-interferred-oblivious} & as blocking, Sec.~\ref{sec:model-interfering-blocking} &\\
\hline
Audsley and Bletsas~\cite{ECRTS-AudsleyB04} & 2004 & & & & \checkmark\\ 
\hline 
Audsley and Bletsas~\cite{RTAS-AudsleyB04} & 2004 & & & & \checkmark\\ 
\hline 
Bletsas and Audsley~\cite{RTCSA-BletsasA05} & 2005 & & & & \checkmark\\
\hline
Bletsas~\cite{bletsas:thesis} & 2007 & & & &\\
\hline 
Ding et al.\cite{DBLP:journals/ieicet/DingTT09} & 2009 &segmented, segmented FP & \multicolumn{2}{C{10cm}|}{enforcement for each computation segment, Sec.~\ref{sec:static-period-enforce}} & \checkmark\\
\hline 
Lakshmanan and Rajkumar~\cite{LR:rtas10} & 2010 & segmented, FP& based on a revised critical instant theorem, Sec.~\ref{sec:existing-exact-special} & based on a revised critical instant theorem & \checkmark\\
\hline
Lakshmanan and Rajkumar~\cite{LR:rtas10} & 2010 & segmented, FP& \multicolumn{2}{C{10cm}|}{slack enforcement, Sec.~\ref{sec:slack-enforce}}&\\
\hline
Kim et al.~\cite{RTSS-KimANR13} & 2013 & segmented, segmented FP & \multicolumn{2}{C{10cm}|}{enforcement for each computation segment, Sec.~\ref{sec:static-period-enforce}} & \checkmark\\
\hline
Liu and Anderson~\cite{DBLP:conf/ecrts/LiuA13} & 2013 & multiprocessor, global FP and EDF &  suspension as computation, Sec.~\ref{sec:model-interferred-oblivious}& quantifying the interference by considering carry-in jobs in multiprocessor scheduling, Sec.~\ref{sec:multiprocessor-HRT} & \checkmark\\
\hline 
Liu et al. \cite{DBLP:conf/ecrts/LiuCH014} & 2014 & dynamic, FP (harmonic)& suspension as computation, Sec.~\ref{sec:model-interferred-oblivious}& no additional impact due to self-suspension&\\ 
\hline 
Liu and Chen~\cite{LiuChen:rtss2014} & 2014 & dynamic, FP& suspension as computation, Sec.~\ref{sec:model-interferred-oblivious}& as carry-in, Sec.~\ref{sec:model-carry-in} &\\
\hline
Chen and Liu~\cite{RTSS-ChenL14} & 2014 & segmented (one-segment), period enforcement&  \multicolumn{2}{C{10cm}|}{speed up factor guarantee with respect to the optimal schedule, Sec.~\ref{sec:static-period-enforce}}&\\
\hline
Huang and Chen \cite{Huang:multiseg}& 2015 & segmented, FP& hybrid approaches, Sec.~\ref{sec:model-interferred-oblivious}-~\ref{sec:model-interferred-hybrid} & segmented structures with dynamic offsets, Sec.~\ref{sec:model-interfering-improving}&\\
\hline
Huang et al.~\cite{huangpass:dac2015} & 2015 & dynamic, FP& suspension as computation, Sec.~\ref{sec:model-interferred-oblivious}& as carry-in,  Sec.~\ref{sec:model-carry-in}&\\
\hline
Nelissen et al.~\cite{ecrts15nelissen} & 2015 & segmented, FP & based on a revised critical instant theorem, Sec.~\ref{sec:existing-exact-special} & suspension by modeling proper jitter \ref{sec:model-interfering-jitter}&\\
\hline
Huang et al.~\cite{WC16-suspend-DATE} & 2016 & segmented, period enforcement& \multicolumn{2}{C{10cm}|}{speed up factor guarantee with respect to the optimal schedule, Sec.~\ref{sec:static-period-enforce}}&\\
\hline
\end{tabular}
}
  \caption{Summary of existing methods for handling self-suspending tasks in scheduling policies and schedulability analyses.}
...
This strategy is sometimes called ``joint'' \cite{bletsas:thesis} but often referred to as the \emph{suspension-oblivious} approach in the literature. It assumes that the self-suspending task $\tau_k$ continues executing on the processor when it self-suspends. Its suspension intervals are thus considered as being preemptible. From an analysis perspective, it is equivalent to replacing the self-suspending task $\tau_k$ by a non-self-suspending task $\tau_k' = (C_k + S_k, D_k, T_k)$ with a worst-case execution time equal to $C_k + S_k$. 
...
For example, Huang et al. in \cite[Theorem 3]{huangpass:dac2015} provided the following necessary condition for any fixed-priority scheduling: \emph{ If there exists a feasible fixed-priority preemptive scheduling
...
As already mentioned in Section~\ref{sec:model-interferred-oblivious}, under fixed-priority scheduling, the self-suspension oblivious approach is an exact analysis for dynamic 
...
After presenting how to model the interfered self-suspending task, we will summarize the existing analyses for modeling the interfering tasks. For analyzing the interfering tasks in the dynamic self-suspending task model, we classify the existing approaches into 
\begin{itemize}
...
self-suspend (see task $\tau_1$ in Figure~\ref{fig:ss-intuition} for a simple example or task $\tau_2$ in Figure~\ref{fig:counterexample-dynamic} in Section~\ref{sec:misconceptions}for a more complicated example). Let $J_i$ denote that jitter on $\tau_i$'s computation segment release. 
