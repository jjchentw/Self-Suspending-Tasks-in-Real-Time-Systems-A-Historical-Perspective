...
\hline
Nelissen et al.~\cite{ecrts15nelissen} & 2015 & segmented, FP & based on a revised critical instant theorem, Sec.~\ref{sec:existing-exact-special} & suspension by modeling proper release jitter \ref{sec:model-interfering-jitter} and enumerating worst-case interferences&\\
\hline
\end{tabular}
}
...
\centering
\scalebox{0.65}{
\begin{tabular}{||C{2.5cm}|C{1cm}|C{3cm}||C{10.1cm}|c|}  
  \hline
papers/methods & year & enforcement & interferring tasks & flawed\\
\hline 
...
enforcement for each computation segment, Sec.~\ref{sec:static-period-enforce} & \checkmark\\
\hline 
Lakshmanan and Rajkumar~\cite{LR:rtas10} & 2010 & segmented, FP& slack enforcement, Sec.~\ref{sec:slack-enforce}&\\
\hline
Kim et al.~\cite{RTSS-KimANR13} & 2013 & segmented, segmented FP & enforcement for each computation segment, Sec.~\ref{sec:static-period-enforce} & \checkmark\\
\hline
Chen and Liu~\cite{RTSS-ChenL14} & 2014 & segmented (one-segment), period enforcement&  speed up factor guarantee with respect to the optimal schedule, Sec.~\ref{sec:static-period-enforce}&\\
\hline
Huang et al.~\cite{WC16-suspend-DATE} & 2016 & segmented, period enforcement& speed up factor guarantee with respect to the optimal schedule, Sec.~\ref{sec:static-period-enforce}&\\
\hline
\end{tabular}
}
