...
An arbitrary task graph can be reduced with some information loss (pessimism) to an instance of the dynamic model. 
%
%with 
...
where $\varphi$ denotes a control flow (path), as a set of nodes traversed~\cite{RTAS-AudsleyB04,bletsas:thesis} for the converted
instance of the dynamic model is safe. However, it is unnecessarily pessimistic, if the maximum execution time and maximum self-suspension 
time are observed in different node paths. A more efficient conversion would use
