...
i.e., there is a deterministic number of self-suspending regions interleaved between portions of processor-based code with single-entry
...
to derive tighter bounds on worst-case response times, where applicable, relative to analysis that only considers the dynamic model. 
...
Under partitioned scheduling, tasks are statically partitioned among processors, i.e., each task is bound to execute on a specific processor and will never migrate to another processor. Different processors can apply different scheduling algorithms. A partitioned algorithm example is partitioned earliest-deadline-first (P-EDF), which uses the EDF algorithm as the per-processor scheduler. 
