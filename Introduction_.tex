...
{\bf Example 6: Hardware Acceleration Using FPGAs or GPUs as Co-Processors}

In many systems, select portions of system code are executed on dedicated hardware co-processors, for speed-up.
Different kinds of co-processor exist. Many embedded systems employ customised application-specific hardware co-processors,
nowadays usually implemented on Field-Programmable Gate Arrays (FPGAs), a popular form of reconfigurable hardware. 
Often, the processor itelf is synthesised on the same FPGA package as the co-processors. 

The use of co-processors in itself, speeds up the application, compared to a fully CPU-based implementation. However, further
performance improvements can be had by having the software task self-suspend, for the duration of the hardware operation, instead of 
busy-waiting. This frees up the processor, to be used by other ready tasks. Then, even in single-CPU systems more than one 
task may be simultuaneously advancing in computation: one task executing on the processor and others on each of the available 
co-processors. This arrangement is called Limited Parallelism and is illustrated by Figure~\ref{fig:example-fpga}.
...
\subfloat[Not use FPGA in parallel (suspension oblivious)]{
   \scalebox{0.8}{
      \begin{tikzpicture}[x=\uxfpga,y=\uy,auto, thick]
        \draw[->] (0,0) -- coordinate (xaxis) (45,0) node[anchor=north]{$t$};
       \node[anchor=east] at (0, 0.75) {CPU};
