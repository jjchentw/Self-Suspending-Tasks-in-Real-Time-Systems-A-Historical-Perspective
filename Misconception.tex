...
However, quantifying the jitter of a self-suspending task $\tau_i$ with $D_i-C_i$ in Section~\ref{sec:wrong-jitter-dynamic}  remains safe for constrained-deadline task systems since the dynamic self-suspending pattern is more general than a segmented self-suspending pattern.
...
Following that idea, Lakshmanan and Rajkumar \cite{LR:rtas10} proposed a pseudo-polynomial worst-case response time analysis for one segmented self-suspending task $\tau_k$ (with one self-suspending interval) assuming that 
\begin{itemize}
...
\centering
\def\uxfpga{0.4cm} 
\subfloat[Scenario 1. Jobs are released as often as possible.]{
\scalebox{0.73}{
	\begin{tikzpicture}[x=\uxfpga,y=\uy,auto, thick]
...
\subfloat[Scenario 2. Jobs are not released as often as possible.]{
\scalebox{0.73}{
	\begin{tikzpicture}[x=\uxfpga,y=\uy,auto, thick]
    \draw[->] (0,0) -- coordinate (xaxis) (39,0) node[anchor=north]{$t$};
    \foreach \x in {0,5,...,35}{
...
{\bf Solutions:} This requires us to revisit the schedulability test of a given segmented fixed-priority assignment. This can be observed as a reduction to 
