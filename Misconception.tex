...
A simple fix can be achieved by applying the following concept to
classify the interfering higher-priority computation segments into two
types: carry-in executions and non-carry-in executions.  When
analyzing the response time of a computation segment, we can
pessimistically account for one higher-priority carry-in computation
segment per task, due to the assumption that the task systems are with
constrained deadlines and the higher-priority computation segments
have to meet their deadlines. With this concept, a fix can be found in~\cite{Kim2016}.
...
\cite{ecrts15nelissen} that analyzes higher-priority self-suspending 
